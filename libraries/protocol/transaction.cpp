...
smoke::protocol::transaction_id_type smoke::protocol::transaction::id() const
{
   auto h = digest();
...
const signature_type& smoke::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)
{
...
signature_type smoke::protocol::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)const
{
   digest_type::encoder enc;
   fc::raw::pack( enc, chain_id );
   fc::raw::pack( enc, *this );
...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         SMOKE_ASSERT( s.check_authority(id) ||
   ...
   {
      SMOKE_ASSERT( s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
   ...
   {
      SMOKE_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id)),
   ...
   ) const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         smoke::protocol::verify_authority( operations, result, get_active, get_owner, get_posting, max_recursion );
...
{ try {
   smoke::protocol::verify_authority( operations, get_signature_keys( chain_id ), get_active, get_owner, get_posting, max_recursion );
