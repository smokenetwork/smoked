...
#include <smoke/plugins/debug_node/debug_node_api.hpp>
...
class debug_private_key_storage : public private_key_storage
{
   public:
      ...
      std::map< smoke::chain::public_key_type, fc::ecc::private_key > key_table;
};

class debug_node_api_impl
{
   public:
      debug_node_api_impl( smoke::app::application& _app );
      ...
      //void debug_push_block( const smoke::chain::signed_block& block );
      ...
      std::shared_ptr< smoke::plugin::debug_node::debug_node_plugin > get_plugin();
...
)
{
   ...
   if( (pubkey == smoke::chain::public_key_type()) || (gen_pub == pubkey) )
   {
      result = gen_priv;
      return;
   }
...
debug_node_api_impl::debug_node_api_impl( smoke::app::application& _app ) : app( _app )
{
#ifdef SMOKE_INIT_PRIVATE_KEY
   fc::ecc::private_key init_key = SMOKE_INIT_PRIVATE_KEY;
...
uint32_t debug_node_api_impl::debug_push_blocks( const std::string& src_filename, uint32_t count, bool skip_validate_invariants )
{
   ...
   std::shared_ptr< smoke::chain::database > db = app.chain_database();
     ...
     )
   {
      ...
      uint32_t skip_flags = smoke::chain::database::skip_nothing;
      if( skip_validate_invariants )
         skip_flags = skip_flags | smoke::chain::database::skip_validate_invariants;
      for( uint32_t i=0; i<count; i++ )
      {
         //fc::optional< smoke::chain::signed_block > block = log.read_block( log.get_block_pos( first_block + i ) );
         uint64_t block_pos = log.get_block_pos( first_block + i );
         if( block_pos == smoke::chain::block_log::npos )
         {
...
{
   return get_plugin()->debug_generate_blocks( debug_key, count, smoke::chain::database::skip_nothing, 0, &key_storage );
}
...
{
   return get_plugin()->debug_generate_blocks_until( debug_key, head_block_time, generate_sparsely, smoke::chain::database::skip_nothing, &key_storage );
}

fc::optional< smoke::chain::signed_block > debug_node_api_impl::debug_pop_block()
{
   std::shared_ptr< smoke::chain::database > db = app.chain_database();
   return db->fetch_block_by_number( db->head_block_num() );
}

/*void debug_node_api_impl::debug_push_block( const smoke::chain::signed_block& block )
{
   app.chain_database()->push_block( block );
}*/

smoke::chain::witness_schedule_object debug_node_api_impl::debug_get_witness_schedule()
{
   return app.chain_database()->get( smoke::chain::witness_schedule_id_type() );
}

smoke::chain::hardfork_property_object debug_node_api_impl::debug_get_hardfork_property_object()
{
   return app.chain_database()->get( smoke::chain::hardfork_property_id_type() );
}
...
std::shared_ptr< smoke::plugin::debug_node::debug_node_plugin > debug_node_api_impl::get_plugin()
{
...
void debug_node_api_impl::debug_set_hardfork( uint32_t hardfork_id )
{
   ...
   if( hardfork_id > SMOKE_NUM_HARDFORKS )
      return;
...
{
   return app.chain_database()->get( smoke::chain::hardfork_property_id_type() ).last_hardfork >= hardfork_id;
}
...
debug_node_api::debug_node_api( const smoke::app::api_context& ctx )
{
...
fc::optional< smoke::chain::signed_block > debug_node_api::debug_pop_block()
{
   return my->debug_pop_block();
}

/*void debug_node_api::debug_push_block( smoke::chain::signed_block& block )
{
   my->debug_push_block( block );
}*/

smoke::chain::witness_schedule_object debug_node_api::debug_get_witness_schedule()
{
   return my->debug_get_witness_schedule();
}

smoke::chain::hardfork_property_object debug_node_api::debug_get_hardfork_property_object()
{
