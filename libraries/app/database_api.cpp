...
{
   public:
      database_api_impl( const smoke::app::api_context& ctx  );
      ~database_api_impl();
      ...
      smoke::chain::database&                _db;
      std::shared_ptr< smoke::follow::follow_api > _follow_api;
...
database_api::database_api( const smoke::app::api_context& ctx )
   : my( new database_api_impl( ctx ) ) {}

database_api::~database_api() {}

database_api_impl::database_api_impl( const smoke::app::api_context& ctx )
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< smoke::follow::follow_api >( ctx );
   }
...
{
   return smoke::protocol::get_config();
}
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_index<smoke::chain::account_member_index>();
...
void database_api::set_pending_payout( discussion& d )const
{
   ...
   if( total_r2 > 0 )
   {
      ...
      if( my->_db.has_hardfork( SMOKE_HARDFORK_0_17__774 ) )
      {
         ...
         vshares = d.net_rshares.value > 0 ? smoke::chain::util::evaluate_reward_curve( d.net_rshares.value ) : 0;
