...
//      open( data_dir, shared_mem_dir, 0, shared_file_size, chainbase::database::read_write );
      open( data_dir, shared_mem_dir, STEEMIT_INIT_SUPPLY, STEEMIT_INIT_SUPPLY_SBD, shared_file_size, chainbase::database::read_write );
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment )
{
  try
  {
     ...
     if( comment.net_rshares > 0 )
     {
        ...
        const share_type reward = util::get_rshare_reward( ctx, has_hardfork( STEEMIT_HARDFORK_0_20) );
