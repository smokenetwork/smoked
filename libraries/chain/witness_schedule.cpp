...
void update_witness_schedule4( database& db )
{
   ...
   for( auto itr = widx.begin();
        ...
        ++itr )
   {
      if( db.has_hardfork( SMOKE_HARDFORK_0_14__278 ) && (itr->signing_key == public_key_type()) )
         continue;
   ...
   while( mitr != pow_idx.end() && selected_miners.size() < wso.max_miner_witnesses )
   {
      ...
      if( selected_voted.find(mitr->id) == selected_voted.end() )
      {
         ...
         if( !( db.has_hardfork( SMOKE_HARDFORK_0_14__278 ) && db.get_witness( mitr->owner ).signing_key == public_key_type() ) )
         {
        ...
        ++sitr )
   {
      ...
      if( db.has_hardfork( SMOKE_HARDFORK_0_14__278 ) && sitr->signing_key == public_key_type() )
   ...
   if( db.has_hardfork( SMOKE_HARDFORK_0_5__54 ) )
   {
...
void update_witness_schedule(database& db)
{
   ...
   {
      if( db.has_hardfork(SMOKE_HARDFORK_0_4) )
      {
      ...
      if( props.num_pow_witnesses == 0 || db.head_block_num() > SMOKE_START_MINER_VOTING_BLOCK )
      {
         ...
         for( auto itr = widx.begin(); itr != widx.end() && (active_witnesses.size() < (SMOKE_MAX_WITNESSES-2)); ++itr )
         {
            if( itr->pow_worker )
               continue;
         ...
         if( sitr != schedule_idx.end() )
         {
            ...
            db.modify( *sitr, [&]( witness_object& wo )
            {
               ...
               if( db.has_hardfork( SMOKE_HARDFORK_0_2 ) )
      ...
      if( props.num_pow_witnesses > SMOKE_MAX_WITNESSES )
      {
         if( itr != pow_idx.end() )
         {
            db.modify( *itr, [&](witness_object& wit )
      ...
      while( itr != pow_idx.end() )
      {
         ...
         if( db.head_block_num() > SMOKE_START_MINER_VOTING_BLOCK || active_witnesses.size() >= SMOKE_MAX_WITNESSES )
            break;
         ++itr;
      }
      ...
      /*
         ...
         for( const string& w : active_witnesses )
            ...
            */
         // active witnesses has exactly SMOKE_MAX_WITNESSES elements, asserted above
         for( size_t i = 0; i < active_witnesses.size(); i++ )
         {
            _wso.current_shuffled_witnesses[i] = active_witnesses[i];
         }
