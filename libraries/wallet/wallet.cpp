...
#include <smoke/wallet/reflect_util.hpp>
...
public:
   ...
   variant info() const
   {
      ...
      result["post_reward_fund"] = fc::variant(_remote_db->get_reward_fund( SMOKE_POST_REWARD_FUND_NAME )).get_object();
      return result;
   }
   ...
   variant_object about() const
   {
      ...
      result["blockchain_version"]       = SMOKE_BLOCKCHAIN_VERSION;
   ...
   bool import_key(string wif_key)
   {
      ...
      smoke::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         smoke::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         smoke::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         smoke::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         smoke::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < SMOKE_MAX_TIME_UNTIL_EXPIRATION );
   ...
   annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false)
   {
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, SMOKE_CHAIN_ID );
      }
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["get_open_orders"] = []( variant result, const fc::variants& a ) {
          ...
          for( const auto& o : orders ) {
             ...
             ss << ' ' << setw( 10 ) << (o.sell_price.base.symbol == SMOKE_SYMBOL ? "SELL" : "BUY");
             ss << "\n";
          }
          return ss.str();
      };
         ...
         for( size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++ )
         {
            if ( i < orders.bids.size() )
            {
               ...
               ss
                  ...
                  << ' ' << setw( spacing ) << asset( orders.bids[i].steem, SMOKE_SYMBOL ).to_string()
            ...
            if ( i < orders.asks.size() )
            {
               ...
               ss << ' ' << setw( spacing ) << orders.asks[i].real_price
                  << ' ' << setw( spacing ) << asset( orders.asks[i].steem, SMOKE_SYMBOL ).to_string()
...
{ try {
   ...
   op.fee = my->_remote_db->get_chain_properties().account_creation_fee * asset( SMOKE_CREATE_ACCOUNT_WITH_STEEM_MODIFIER, SMOKE_SYMBOL );
...
annotated_signed_transaction wallet_api::vote( string voter, string author, string permlink, int16_t weight, bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * SMOKE_1_PERCENT;
...
annotated_signed_transaction      wallet_api::send_private_message( string from, string to, string subject, string body, bool broadcast ) {
   ...
   op.id = SMOKE_PRIVATE_MESSAGE_COP_ID;
