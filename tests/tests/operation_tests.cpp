...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( SBD_SYMBOL, SMOKE_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is SMOKE" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success transferring SMOKE to self" );
      op.to = "alice";
      ...
      BOOST_TEST_MESSAGE( "--- success transferring SMOKE to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing SMOKE to self" );
      op.to = "alice";
...
//      BOOST_REQUIRE( db.get_savings_withdraw( "alice", op.request_id ).complete == db.head_block_time() + SMOKE_SAVINGS_WITHDRAW_TIME );
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing SMOKE to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
